...
from AWGCommunicator import AWGCommunicator
...
class AWG_GUI_handler:
    ...
    def run(self, channel, combined):
        if channel == 1:
            ...
            amplitude_dict = {}
            if not combined:    
                amplitude_dict['start_amp'] = float(getattr(self.gui, f'ch{channel}_start_amp').text().strip())
                amplitude_dict['stop_amp'] = float(getattr(self.gui, f'ch{channel}_stop_amp').text().strip())
                amplitude_dict['step_amp'] = float(getattr(self.gui, f'ch{channel}_step_amp').text().strip())

            if combined:
                amplitude_dict['start_amp'] = float(self.gui.combined_start_amp.text().strip())
                amplitude_dict['stop_amp'] = float(self.gui.combined_stop_amp.text().strip())
                amplitude_dict['step_amp'] = float(self.gui.combined_step_amp.text().strip())
