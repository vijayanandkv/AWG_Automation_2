...
class AWGGui(QMainWindow):
    ...
    def init_channel_1_tab(self):
        ...
        # Parameters group
        self.ch1_param_group = QGroupBox("Parameters")
        param_form = QFormLayout()
        
        # Common parameters
        self.ch1_freq_input = QLineEdit()
        self.ch1_amp_input = QLineEdit()
        param_form.addRow("Frequency (GHz):", self.ch1_freq_input)
        param_form.addRow("Amplitude (V):", self.ch1_amp_input)
        
        # PRBS parameters
        self.ch1_prbs_order_input = QLineEdit()
        self.ch1_prbs_repetition_rate = QLineEdit()
        param_form.addRow("Order:", self.ch1_prbs_order_input)
        param_form.addRow("Repetition Rate", self.ch1_prbs_repetition_rate)
        
        # Noise parameters
        self.ch1_noise_bandwidth_input = QLineEdit()
        param_form.addRow("Bandwidth (Hz):", self.ch1_noise_bandwidth_input)
        
        # LFM parameters
        self.ch1_lfm_center_freq = QLineEdit()
        self.ch1_lfm_pulse_width = QLineEdit()
        self.ch1_lfm_bandwidth = QLineEdit()
        param_form.addRow("Center Freq (GHz):", self.ch1_lfm_center_freq)
        param_form.addRow("Pulse width (ns):", self.ch1_lfm_pulse_width)
        param_form.addRow("Bandwidth (GHz):", self.ch1_lfm_bandwidth)

        self.ch1_generate_wave_btn = QPushButton(CONFIG['buttons']['Generate_waveform']['label'])
        param_form.addRow(self.ch1_generate_wave_btn)
        self.ch1_param_group.setLayout(param_form)
        side_panel.addWidget(self.ch1_param_group)

        # Segment group
        ch1_segment_group = QGroupBox()
        segment_layout = QFormLayout()
        button_layout = QHBoxLayout()
        self.ch1_segment_id = QLineEdit()
        self.ch1_num_samples = QLineEdit()
        self.ch1_define_segment_btn = QPushButton(CONFIG["buttons"]["Define_segment"]["label"])
        self.ch1_delete_segment_btn = QPushButton(CONFIG["buttons"]["Delete_segment"]["label"])
        segment_layout.addRow("Segment ID", self.ch1_segment_id)
        segment_layout.addRow("Number of samples", self.ch1_num_samples)
        button_layout.addWidget(self.ch1_define_segment_btn)
        button_layout.addWidget(self.ch1_delete_segment_btn)
        segment_layout.addRow(button_layout)
        ch1_segment_group.setLayout(segment_layout)
        side_panel.addWidget(ch1_segment_group)

        # File path group
        ch1_file_path_group = QGroupBox()
        file_path_layout = QFormLayout()
        self.ch1_file_path = QLineEdit()
        self.ch1_file_send_btn = QPushButton(CONFIG["buttons"]['Define_file_path']['label'])
        file_path_layout.addRow("Enter file path", self.ch1_file_path)
        file_path_layout.addRow(self.ch1_file_send_btn)
        ch1_file_path_group.setLayout(file_path_layout)
        side_panel.addWidget(ch1_file_path_group)
        ...
        run_layout.addWidget(self.ch1_run_btn)
        run_layout.addWidget(self.ch1_abort_btn)
        ch1_run_group.setLayout(run_layout)
        side_panel.addWidget(ch1_run_group)

        #ouput control group
        '''output_control_group = QGroupBox()
        output_control_layout = QFormLayout()
        output1_control_hbox = QHBoxLayout()
        output2_control_hbox = QHBoxLayout()

        self.output1_enable_btn = QPushButton(CONFIG["buttons"]['output1_enable']['label'])
        self.output1_disable_btn = QPushButton(CONFIG['buttons']['output1_disable']['label'])
        self.output2_enable_btn = QPushButton(CONFIG['buttons']['output2_enable']['label'])
        self.output2_disable_btn = QPushButton(CONFIG['buttons']['output2_disable']['label'])

        output1_control_hbox.addWidget(self.output1_enable_btn)
        output1_control_hbox.addWidget(self.output1_disable_btn)
        output2_control_hbox.addWidget(self.output2_enable_btn)
        output2_control_hbox.addWidget(self.output2_disable_btn)
        output_control_layout.addRow(output1_control_hbox)
        output_control_layout.addRow(output2_control_hbox)
        output_control_group.setLayout(output_control_layout)
        side_panel.addWidget(output_control_group)'''

        # Create the plot canvas and axes for channel 1
        plot_group = QGroupBox("Sample waveform")
        plot_layout = QHBoxLayout()

        self.ch1_canvas = FigureCanvas(Figure(figsize=(5, 3)))
        self.ch1_ax = self.ch1_canvas.figure.subplots()
        ...
        # Connect signals
        self.ch1_generate_wave_btn.clicked.connect(lambda: self.handle_generate_wave(1))
        self.ch1_define_segment_btn.clicked.connect(lambda: self.handle_segment_define(1))
        self.ch1_delete_segment_btn.clicked.connect(lambda: self.handle_segment_delete(1))
        self.ch1_file_send_btn.clicked.connect(lambda: self.handle_send_file(1))
        self.ch1_run_btn.clicked.connect(lambda: self.handle_run(1))
        self.ch1_abort_btn.clicked.connect(lambda: self.handle_abort(1))

        '''self.output1_enable_btn.clicked.connect(lambda:self.handle_output_state(1, 1))
        self.output1_disable_btn.clicked.connect(lambda:self.handle_output_state(1, 0))
        self.output2_enable_btn.clicked.connect(lambda:self.handle_output_state(2, 1))
        self.output2_disable_btn.clicked.connect(lambda:self.handle_output_state(2, 0))'''
    ...
    def init_channel_2_tab(self):
        ...
        # Parameters group
        self.ch2_param_group = QGroupBox("Parameters")
        param_form = QFormLayout()
        
        # Common parameters
        self.ch2_freq_input = QLineEdit()
        self.ch2_amp_input = QLineEdit()
        param_form.addRow("Frequency (GHz):", self.ch2_freq_input)
        param_form.addRow("Amplitude (V):", self.ch2_amp_input)
        
        # PRBS parameters
        self.ch2_prbs_order_input = QLineEdit()
        self.ch2_prbs_repetition_rate = QLineEdit()
        param_form.addRow("Order:", self.ch2_prbs_order_input)
        param_form.addRow("Repetition rate", self.ch2_prbs_repetition_rate)
        
        # Noise parameters
        self.ch2_noise_bandwidth_input = QLineEdit()
        param_form.addRow("Bandwidth (Hz):", self.ch2_noise_bandwidth_input)
        
        # LFM parameters
        self.ch2_lfm_center_freq = QLineEdit()
        self.ch2_lfm_pulse_width = QLineEdit()
        self.ch2_lfm_bandwidth = QLineEdit()
        param_form.addRow("Center Freq (GHz):", self.ch2_lfm_center_freq)
        param_form.addRow("Pulse width (ns):", self.ch2_lfm_pulse_width)
        param_form.addRow("Bandwidth (GHz):", self.ch2_lfm_bandwidth)

        self.ch2_generate_wave_btn = QPushButton(CONFIG['buttons']['Generate_waveform']['label'])
        param_form.addRow(self.ch2_generate_wave_btn)
        self.ch2_param_group.setLayout(param_form)
        side_panel.addWidget(self.ch2_param_group)

        # Segment group
        ch2_segment_group = QGroupBox()
        segment_layout = QFormLayout()
        button_layout = QHBoxLayout()
        self.ch2_segment_id = QLineEdit()
        self.ch2_num_samples = QLineEdit()
        self.ch2_define_segment_btn = QPushButton(CONFIG["buttons"]["Define_segment"]["label"])
        self.ch2_delete_segment_btn = QPushButton(CONFIG["buttons"]["Delete_segment"]["label"])
        segment_layout.addRow("Segment ID", self.ch2_segment_id)
        segment_layout.addRow("Number of samples", self.ch2_num_samples)
        button_layout.addWidget(self.ch2_define_segment_btn)
        button_layout.addWidget(self.ch2_delete_segment_btn)
        segment_layout.addRow(button_layout)
        ch2_segment_group.setLayout(segment_layout)
        side_panel.addWidget(ch2_segment_group)

        # File path group
        ch2_file_path_group = QGroupBox()
        file_path_layout = QFormLayout()
        self.ch2_file_path = QLineEdit()
        self.ch2_file_send_btn = QPushButton(CONFIG["buttons"]['Define_file_path']['label'])
        file_path_layout.addRow("Enter file path", self.ch2_file_path)
        file_path_layout.addRow(self.ch2_file_send_btn)
        ch2_file_path_group.setLayout(file_path_layout)
        side_panel.addWidget(ch2_file_path_group)
        ...
        run_layout.addWidget(self.ch2_run_btn)
        run_layout.addWidget(self.ch2_abort_btn)
        ...
        plot_group = QGroupBox("Sample waveform")
        plot_layout = QHBoxLayout()
        self.ch2_canvas = FigureCanvas(Figure(figsize=(5, 3)))
        self.ch2_ax = self.ch2_canvas.figure.subplots()
        ...
        # Connect signals
        self.ch2_generate_wave_btn.clicked.connect(lambda: self.handle_generate_wave(2))
        self.ch2_define_segment_btn.clicked.connect(lambda: self.handle_segment_define(2))
        self.ch2_delete_segment_btn.clicked.connect(lambda: self.handle_segment_delete(2))
        self.ch2_file_send_btn.clicked.connect(lambda: self.handle_send_file(2))
        self.ch2_run_btn.clicked.connect(lambda: self.handle_run(2))
    ...
    def handle_generate_waveform(self, channel):
        if channel == 1:
            ...
            start_f = float(self.ch1_start_freq.text().strip())
            stop_f= float(self.ch1_stop_freq.text().strip())
            step_f = float(self.ch1_step_freq.text().strip())
            
            for f in np.arange(start_f, stop_f + 0.0001, step_f):
                t, w = self.generator.sinusoidal(frequency=f)
                self.save_waveform_to_csv(waveform_data= w, waveform_type='Sine', frequency= f, channel=1, folder= ch1_folder)

        elif channel == 2:
            ch2_folder = QFileDialog.getExistingDirectory(self, "Select Folder to Save CSV")
            self.generator = WaveformGenerator('1.00.0')
            start_f = float(self.ch2_start_freq.text().strip())
            stop_f= float(self.ch2_stop_freq.text().strip())
            step_f = float(self.ch2_step_freq.text().strip())

            for f in np.arange(start_f, stop_f + 0.0001, step_f):
                t, w = self.generator.sinusoidal(frequency=f)
                self.save_waveform_to_csv(waveform_data= w, waveform_type='Sine', frequency= f, channel=2, folder= ch2_folder)
    ...
    def run(self, channel):
        ...
        if channel == 1:
            ...
            stop_f= float(self.ch1_stop_freq.text().strip())
            step_f = float(self.ch1_step_freq.text().strip())
            start_amp = float(self.ch1_start_amplitude.text().strip())
            stop_amp= float(self.ch1_stop_amplitude.text().strip())
            step_amp = float(self.ch1_step_amplitude.text().strip())

            for f in np.arange(start_f, stop_f + 0.0001, step_f):
                try:                    
                    file_path = self.folder + f"sine_{f}GHz_ch{channel}.csv" 
            ...
            stop_f= float(self.ch2_stop_freq.text().strip())
            step_f = float(self.ch2_step_freq.text().strip())
            start_amp = float(self.ch2_start_amplitude.text().strip())
            stop_amp= float(self.ch2_stop_amplitude.text().strip())
            step_amp = float(self.ch2_step_amplitude.text().strip())
            for f in np.arange(start_f, stop_f + 0.0001, step_f):
                try:
                    ...
                    imp_log = self.awg.import_file(file_path)
                    self.log_box.append(f"{seg_log}")
                    self.log_box.append(f"{imp_log}")

                    for amplitude in np.arange(start_amp, stop_amp + 0.001, step_amp):
                        abort_log = self.awg.abort_wave_generation(channel=channel)
                        output_log = self.awg.set_output_state(channel=channel, state=0)
                        out_volt_log = self.awg.set_output_voltage_custom(channel=channel, value=amplitude)
                        self.log_box.append(f"{output_log}")
    ...
    def update_waveform_inputs(self, waveform_type, channel):
        ...
        if channel == 1:
            # Channel 1 inputs
            self.ch1_freq_input.setEnabled(waveform_type == "Sine")
            self.ch1_amp_input.setEnabled(True)
            self.ch1_prbs_order_input.setEnabled(waveform_type == "PRBS")
            self.ch1_prbs_repetition_rate.setEnabled(waveform_type == "PRBS")
            self.ch1_noise_bandwidth_input.setEnabled(waveform_type == "Noise")
            self.ch1_lfm_center_freq.setEnabled(waveform_type == "LFM")
            self.ch1_lfm_pulse_width.setEnabled(waveform_type == "LFM")
            self.ch1_lfm_bandwidth.setEnabled(waveform_type == "LFM")
        elif channel == 2:
            # Channel 2 inputs
            self.ch2_freq_input.setEnabled(waveform_type == "Sine")
            self.ch2_amp_input.setEnabled(True)
            self.ch2_prbs_order_input.setEnabled(waveform_type == "PRBS")
            self.ch2_prbs_repetition_rate.setEnabled(waveform_type == "PRBS")
            self.ch2_noise_bandwidth_input.setEnabled(waveform_type == "Noise")
            self.ch2_lfm_center_freq.setEnabled(waveform_type == "LFM")
            self.ch2_lfm_pulse_width.setEnabled(waveform_type == "LFM")
            self.ch2_lfm_bandwidth.setEnabled(waveform_type == "LFM")
    ...
    def get_channel_inputs(self, channel):
        ...
        if channel == 1:
            return {
                ...
                'freq_input': self.ch1_freq_input,
                'amp_input': self.ch1_amp_input,
                'prbs_order_input': self.ch1_prbs_order_input,
                'prbs_repetition_rate': self.ch1_prbs_repetition_rate,
                'noise_bandwidth_input': self.ch1_noise_bandwidth_input,
                'lfm_center_freq': self.ch1_lfm_center_freq,
                'lfm_pulse_width': self.ch1_lfm_pulse_width,
                'lfm_bandwidth': self.ch1_lfm_bandwidth,
                'canvas': self.ch1_canvas,
                'ax': self.ch1_ax,
                'segment_id': self.ch1_segment_id,
                'num_samples': self.ch1_num_samples,
                'file_path': self.ch1_file_path
            }
        elif channel == 2:
            return {
                'waveform_selector': self.ch2_waveform_selector,
                'freq_input': self.ch2_freq_input,
                'amp_input': self.ch2_amp_input,
                'prbs_order_input': self.ch2_prbs_order_input,
                'prbs_repetition_rate': self.ch2_prbs_repetition_rate,
                'noise_bandwidth_input': self.ch2_noise_bandwidth_input,
                'lfm_center_freq': self.ch2_lfm_center_freq,
                'lfm_pulse_width': self.ch2_lfm_pulse_width,
                'lfm_bandwidth': self.ch2_lfm_bandwidth,
                'canvas': self.ch2_canvas,
                'ax': self.ch2_ax,
                'segment_id': self.ch2_segment_id,
                'num_samples': self.ch2_num_samples,
                'file_path': self.ch2_file_path
            }

    def validate_inputs(self, inputs, waveform_type):
        """Validate inputs based on waveform type"""
        if waveform_type == "Sine":
            if not inputs['freq_input'].text().strip() or not inputs['amp_input'].text().strip():
                return False, "Frequency and Amplitude are required for Sine waveform."
        elif waveform_type == "PRBS":
            if not inputs['prbs_order_input'].text().strip() or not inputs['amp_input'].text().strip() or not inputs['prbs_repetition_rate'].text().strip():
                return False, "PRBS requires order, repetition rate and amplitude."
        elif waveform_type == "LFM":
            if (not inputs['lfm_center_freq'].text().strip() or 
                not inputs['lfm_pulse_width'].text().strip() or 
                not inputs['lfm_bandwidth'].text().strip() or
                not inputs['amp_input'].text().strip()):
                return False, "LFM requires center frequency, pulse width, and bandwidth."
        return True, ""

    def handle_generate_wave(self, channel):
        """Generate waveform for specified channel"""
        inputs = self.get_channel_inputs(channel)
        waveform_type = inputs['waveform_selector'].currentText()
        
        # Validate inputs
        is_valid, error_msg = self.validate_inputs(inputs, waveform_type)
        if not is_valid:
            self.log_box.append(f"⚠️ {error_msg}")
            return

        self.generator = WaveformGenerator(ip_address= 'WINDOWS-EJL97HL')

        try:
            amp = float(inputs['amp_input'].text().strip())
            # Generate waveform based on selection
            if waveform_type == "Sine":
                freq = float(inputs['freq_input'].text().strip())
                
                t, f = self.generator.sinusoidal(amplitude=amp, frequency=freq)

            elif waveform_type == "PRBS":
                order = inputs['prbs_order_input'].text().strip()
                repetition = inputs['prbs_repetition_rate'].text().strip()
                t, f = self.generator.PRBS(amplitude=amp, order=order, repetition_rate=repetition)

            elif waveform_type == "Noise":
                t, f = self.generator.generate_noise()

            elif waveform_type == "LFM":
                center_freq = float(inputs['lfm_center_freq'].text().strip())
                pulse_width = float(inputs['lfm_pulse_width'].text().strip())
                bandwidth = float(inputs['lfm_bandwidth'].text().strip())
                t, f = self.generator.generate_lfm(amplitude=amp, center_freq=center_freq, pulse_width=pulse_width, bandwidth=bandwidth)

            else:
                self.log_box.append("❌ Unknown waveform selected.")
                return

            # Plot waveform
            inputs['ax'].clear()
            inputs['ax'].plot(t * 1e9, f, color='blue')
            inputs['ax'].set_title(f"Channel {channel} - {waveform_type} Waveform")
            inputs['ax'].set_xlabel("Time (ns)")
            inputs['ax'].set_ylabel("Amplitude (V)")
            inputs['canvas'].draw()
            self.save_waveform_to_csv(f, waveform_type=waveform_type, channel=channel)
    ...
    def save_waveform_to_csv(self, waveform_data, waveform_type, frequency, channel, folder):
        ...
        filename = f"{waveform_type.lower()}_{frequency}GHz_ch{channel}.csv"
    ...
    def handle_segment_define(self, channel):
        """Define segment for specified channel"""
        if not self.check_awg_connection():
            return
        
        inputs = self.get_channel_inputs(channel)
        try:
            segment_id_text = inputs['segment_id'].text().strip()
            num_samples_text = inputs['num_samples'].text().strip()
            
            if not segment_id_text or not num_samples_text:
                self.log_box.append("❌ Please enter both segment ID and number of samples")
                return
                
            segment_id = int(segment_id_text)
            n_sample = int(num_samples_text)
            
            self.awg.define_segment(channel=channel, segment_id=segment_id, n_sample=n_sample)
            self.log_box.append(f"✅ Segment {segment_id} defined for channel {channel}")
        except ValueError:
            self.log_box.append("❌ Invalid segment ID or number of samples")
        except Exception as e:
            self.log_box.append(f"❌ Failed to define segment: {e}")

    def handle_segment_delete(self, channel):
        """Delete segment for specified channel"""
        if not self.check_awg_connection():
            return
        ...
        try:
            ...
            if not segment_id_text:
                self.log_box.append("❌ Please enter segment ID")
                return
                
            segment_id = int(segment_id_text)
            self.awg.delete_segment(channel=channel, id=segment_id)
            self.log_box.append(f"✅ Segment {segment_id} deleted from channel {channel}")
        except ValueError:
            self.log_box.append("❌ Invalid segment ID")
        except Exception as e:
            self.log_box.append(f"❌ Failed to delete segment: {e}")

    def handle_send_file(self, channel):
        """Send file to AWG for specified channel"""
        if not self.check_awg_connection():
            return
        
        inputs = self.get_channel_inputs(channel)
        try:
            filepath = inputs['file_path'].text().strip()
            
            if not filepath:
                self.log_box.append("❌ Please enter file path")
                return
                
            if not os.path.exists(filepath):
                self.log_box.append(f"❌ File does not exist: {filepath}")
                return
                
            self.awg.import_file(filepath)
            self.log_box.append(f"✅ File path {filepath} sent to device for channel {channel}")
        except Exception as e:
            self.log_box.append(f"❌ Failed to send file: {e}")

    def handle_run(self, channel):
        """Start waveform generation for specified channel"""
        if not self.check_awg_connection():
            return
        
        try:
            self.awg.initiate_signal(channel=channel)
            self.log_box.append(f"✅ Waveform generation started for channel {channel}")
        except Exception as e:
            self.log_box.append(f"❌ Failed to start waveform generation: {e}")
